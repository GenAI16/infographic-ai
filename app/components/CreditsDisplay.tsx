...
import type { CreditsResponse } from '@/lib/database.types';
...
export function CreditsDisplay({ onCreditsChange, refreshTrigger }: CreditsDisplayProps) {
  ...
  useEffect(() => {
    async function fetchCredits() {
      ...
      if (data && onCreditsChange) {
        onCreditsChange(data.balance);
      }
    }
    
    fetchCredits();
  }, [refreshTrigger, onCreditsChange]);
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200 rounded-full">
      ...
      <span className="text-amber-600 text-xs hidden sm:inline">credits</span>
    </div>
  );
}
